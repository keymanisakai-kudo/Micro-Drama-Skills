...
description: 整部作品提交技能。读取每集 seedance_tasks.json 或全剧 seedance_project_tasks.json，按 Seedance 任务提交协议批量推送到 /api/tasks/push，并生成提交报告。关键词：提交任务、Seedance、批量推送、整部作品、task submit、api/tasks/push。
---
...
输入来源优先级：
1. `seedance_project_tasks.json`（推荐）
2. 各集 `episodes/EPxx/seedance_tasks.json` 聚合
...
- `realSubmit` (默认 `false`)
- `priority` (默认 `1`)
- `tags` (建议包含 `project_id`,`EPxx`,`A|B`,`GRIDn`)
...
```
DM-002_tjkc/
├── seedance_project_tasks.json          # 可选，推荐
├── episodes/
│   ├── EP01/seedance_tasks.json
│   ├── EP02/seedance_tasks.json
│   └── ...
...
2. 若未指定，使用 `/data/dongman/projects/index.json` 最新项目
